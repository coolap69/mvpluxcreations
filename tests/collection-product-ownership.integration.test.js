...
Deno.test('Dashboard and storefront use the same normalized Product assignment and separate destructive actions', () => {
  ...
  assert(adminSource.includes('Remove from ${category.parentKey ? \'Child Group\' : \'Collection\'}') && adminSource.includes('Save All Live Changes'), 'Dashboard must expose relationship removal and the explicit batch Save Live action separately');
