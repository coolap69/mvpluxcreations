...
let adminPublishedFileState = { reachable: false, publishedAt: null, commitHash: '', source: '' };
...
let adminRecoveryBackupAvailable = false;
let adminRecoveryStateLoaded = false;
let imageInventoryLoaded = false;
...
const ADMIN_DASHBOARD_COLLECTIONS = [
  ...
  'globalDisplaySettings', 'lastPublishedSnapshot', 'liveContentEnabled', 'liveContentRevision',
  'livePublishedAt', 'products', 'publishHistory', 'savedForLaterProducts', 'schemaVersion'
];
...
function receiveAdminSaveNotice(message) {
  ...
  adminCrossTabRefreshTimer = setTimeout(async () => {
    try {
      const settings = await loadAdminLiveSettings([...adminWorkingCollectionsLoaded], { replace: true });
      if (!settings) return;
...
async function loadAdminLiveSettings(keys = ADMIN_DASHBOARD_COLLECTIONS, { replace = false } = {}) {
  ...
  adminLiveSettings = { ...(replace ? {} : adminLiveSettings || {}), ...(globalRow?.edits || {}) };
  adminLiveRevision = Number(globalRow?.revision) || 0;
  requestedKeys.forEach((key) => adminWorkingCollectionsLoaded.add(key));
  return adminLiveSettings;
}

async function ensureAdminWorkingCollections(keys = []) {
  const missing = [...new Set(keys)].filter((key) => !adminWorkingCollectionsLoaded.has(key));
  if (missing.length) await loadAdminLiveSettings(missing);
  return adminLiveSettings;
}

async function loadAdminRecoveryState() {
  if (adminRecoveryStateLoaded) return adminSiteEditRows;
  const result = await callAdminPublisher({ action: 'recovery-state' });
  const rows = Array.isArray(result.rows) ? result.rows : [];
  adminSiteEditRows = rows;
  const globalRow = rows.find((row) => row.page_key === 'admin-global');
  if (globalRow?.edits) {
    adminLiveSettings = globalRow.edits;
    adminLiveRevision = Number(globalRow.revision) || adminLiveRevision;
    adminRecoveryBackupAvailable = Boolean(globalRow.edits.adminPublishingMigrationBackupV1);
  }
  adminHomepageLiveEdits = rows.find((row) => row.page_key === 'index.html')?.edits || {};
  adminPageLiveEdits = Object.fromEntries(rows
    .filter((row) => row.page_key !== 'admin-global' && row.edits && typeof row.edits === 'object')
    .map((row) => [String(row.page_key || '').toLowerCase(), row.edits]));
  adminRecoveryStateLoaded = true;
  return rows;
}
...
function readDeletedCategories() {
  return [...getAdminLiveValue('deletedCategories', [])];
}

function categoryDefinitionsForAdmin() {
  const categories = readAdminCategories();
  if (Object.keys(categories).length) return Object.values(categories);
  return Object.values(adminArchitectureState?.candidate?.categories || {});
}
...
function refreshUntouchedProductFormFields(form, latestRecord, dirtyFields = form?._adminDirtyFields || new Set()) {
  ...
  ['title', 'description', 'funFact', 'cutoutImage', 'backgroundImage', 'originalHeight', 'priceOverride', 'cutoutHeight', 'cutoutLeft', 'cutoutBottom', 'logoWidth', 'logoTop', 'stageBackgroundPosition']
    .forEach((fieldName) => {
...
const ADMIN_COLLECTION_STORAGE_KEYS = {
  ...
  deletedCategories: 'mvpluxDeletedCategories',
...
async function saveAdminCollectionOperations(operations) {
  ...
  const invalidImages = requested.flatMap((operation) => (adminStateUtils.invalidAdminCollectionImageReferences?.(
    ...
    operation.type === 'value' ? operation.value : operation.patch,
    [operation.entryKey].filter(Boolean)
  ) || []).filter((failure) => {
    if (operation.type !== 'record' || !operation.baseRecord) return true;
    const relativePath = failure.path.split('.').slice(1);
    return valueAtPath(operation.baseRecord, relativePath) !== failure.value;
  }));
  if (invalidImages.length) {
    const fields = invalidImages.map((item) => item.path).join(', ');
    const message = `Save stopped. ${invalidImages[0].reason} Invalid field${invalidImages.length === 1 ? '' : 's'}: ${fields}.`;
    adminLastSaveSucceeded = false;
    adminLastSaveError = message;
    setStatus(message);
    return { ok: false, validation: true, invalidImages };
  }
  adminSavePending += 1;
  renderAdminDiagnostics();
  ...
  const save = async () => {
    try {
      ...
      const { adminPublishingMigrationBackupV1: latestRecoveryBackup } = latest.edits || {};
      adminRecoveryBackupAvailable = adminRecoveryBackupAvailable || Boolean(latestRecoveryBackup);
      adminLiveSettings = { ...(adminLiveSettings || {}) };
      adminLiveRevision = latest.revision;
      adminPotentiallyStale = false;
      if (conflicts.length) {
        adminLastSaveSucceeded = false;
...
function renderAdminExportPreview(exportData = buildAdminExport()) {
  ...
  const json = JSON.stringify(redactAdminEmbeddedImages(exportData), null, 2);
...
function semanticComparisonContext() {
  return {
    ...
    categorySettings: Object.fromEntries(
      Object.entries(readAdminCategories()).map(([key, category]) => [key, category?.displaySettings || {}])
    )
  };
}

function semanticProductEqual(left, right) {
  if (!left || !right) return left === right;
  return adminStateUtils?.semanticProductEqual
    ? adminStateUtils.semanticProductEqual(left, right, semanticComparisonContext())
    : JSON.stringify(left) === JSON.stringify(right);
}

function semanticValuesEqual(left, right) {
  return adminStateUtils?.semanticValuesEqual
    ? adminStateUtils.semanticValuesEqual(left, right)
    : JSON.stringify(left) === JSON.stringify(right);
}
...
function buildCurrentPublishSnapshot() {
  ...
  return {
    ...
    deletedCategories: readDeletedCategories().sort(),
...
function buildNormalizedPublishSnapshot() {
  ...
  Object.keys(products).forEach((slug) => {
    ...
    const categories = (product.categories || []).filter((key) => !deletedCategories.has(key));
    product.categories = categories;
    product.categoryOrder = Object.fromEntries(Object.entries(product.categoryOrder || {}).filter(([key]) => categories.includes(key)));
  });
  ...
  deletedCategories.forEach((key) => delete categories[key]);
  const categoryDisplayCards = { ...(published.categoryDisplayCards || {}) };
  deletedCategories.forEach((key) => {
    delete categoryDisplayCards[categoryCardSlugForKey(key)];
    delete categoryDisplayCards[`${key}-category-card`];
  });
  ...
  return {
    ...
    products,
    categories,
    deletedCategories: [...deletedCategories].sort(),
    categoryDisplayCards,
    ...
    ignoredImagePaths: [...new Set(readImageDraftPaths('ignoredImagePaths'))].sort(),
    homepageCategoryOrder: homepageOrder?.type === 'homepageCategoryOrder' && Array.isArray(homepageOrder.rows)
      ? homepageOrder.rows.map((row) => row.filter((value) => !deletedCategories.has(value)
        && !deletedCategories.has(ADMIN_CATEGORY_CARD_MAP[value])
        && ![...deletedCategories].some((key) => value === `${key}-category-card`))) : [],
...
function generatePublishChanges(before, after) {
  ...
  if (!semanticValuesEqual(before?.priceSettings || {}, after?.priceSettings || {})) {
  ...
  [...new Set([...Object.keys(beforeVisualStates), ...Object.keys(afterVisualStates)])].sort().forEach((pageKey) => {
    if (!semanticValuesEqual(beforeVisualStates[pageKey] || {}, afterVisualStates[pageKey] || {})) {
  ...
  const beforeDeletedCategories = new Set(before?.deletedCategories || []);
  (after?.deletedCategories || []).forEach((key) => {
    if (!beforeDeletedCategories.has(key)) lines.push(`Deleted category and removed its product assignments: ${key}`);
  });
  if (!semanticValuesEqual(before?.globalDisplaySettings || {}, after?.globalDisplaySettings || {})) {
  ...
  [...new Set([...Object.keys(beforePageContent), ...Object.keys(afterPageContent)])].sort().forEach((pageKey) => {
    if (!semanticValuesEqual(beforePageContent[pageKey] || {}, afterPageContent[pageKey] || {})) {
...
function normalizePublishedBaseline(snapshot) {
  ...
  baseline.deletedCategories = Array.isArray(snapshot.deletedCategories) ? [...snapshot.deletedCategories] : [];
...
async function loadPublicLivePublishBaseline({ timeoutMs = 3500 } = {}) {
  const client = getAdminClient();
  ...
  try {
    const { data, error } = await Promise.race([client.rpc('get_public_site_snapshot'), timeout]);
    if (error) throw error;
    if (!data?.snapshot || data.snapshot.version !== 1 || !data.snapshot.products || !data.snapshot.categoryDisplayCards) {
      throw new Error('Supabase returned no activated valid public snapshot.');
    }
    return data;
  } finally {
...
async function loadPublishedPublishBaseline() {
  try {
    let value;
    let source = 'supabase-live';
    try {
      value = await loadPublicLivePublishBaseline();
    } catch (_liveError) {
    ...
    adminPublishedFileState = { reachable: false, publishedAt: null, commitHash: '', source: '' };
  }
  if (['loading', 'loaded'].includes(adminAreaLoadState.get('advanced')?.status)) renderAdminDiagnostics();
...
function productLifecycleState(product, saved, archived) {
  ...
  const publishedSnapshot = adminPublishedBaseline || adminLastSuccessfulSnapshot;
  const published = publishedSnapshot?.products?.[product.slug];
  if (!published) return { key: 'draft', label: 'DRAFT SAVED — PRIVATE' };
  if (semanticProductEqual(current, published)) return { key: 'published', label: 'LIVE' };
...
function getProductLifecycleCounts() {
  ...
  return {
    ...
    waiting: states.filter((state) => ['draft', 'unpublished'].includes(state.key)).length,
...
function renderAdminDiagnostics() {
  ...
  container.innerHTML = `
    <dl>
      ...
      <div><dt>Published website data</dt><dd>${adminPublishedFileState.reachable ? `Available · ${adminPublishedFileState.source === 'supabase-live' ? 'Supabase live' : 'static emergency fallback'}` : 'Unavailable'}</dd></div>
    </dl>
  `;
}
...
async function undeployedLiveSnapshotImages(snapshot) {
  const previous = publishSnapshotImagePaths(adminPublishedBaseline || buildDefaultPublishBaseline());
  const changedPaths = [...publishSnapshotImagePaths(snapshot)].filter((path) => !previous.has(path));
  if (!changedPaths.length) return [];
  if (!imageInventoryLoaded) await loadImageDraftInventory({ renderInbox: false });
  return changedPaths.filter((path) => localOnlyImagePaths.has(path) || !repositoryImagePaths.has(path));
}

async function savePublicLiveSnapshot(snapshot, label, statusTarget = null, { onProgress = null } = {}) {
  ...
  report('SAVING LIVE…', 'saving-live');
  try {
    const undeployedImages = await undeployedLiveSnapshotImages(snapshot);
    if (undeployedImages.length) {
      report(`SAVE FAILED — WEBSITE NOT CHANGED. New physical image file requires the static asset publisher: ${undeployedImages[0]}`, 'failed');
      return false;
    }
    const result = await callAdminPublisher({
      action: 'save-live',
      title: String(label || 'Admin live content').slice(0, 120),
      snapshot,
      expectedRevision: adminLiveRevision,
      expectedLiveRevision: Number(adminLiveSettings?.liveContentRevision) || 0
    });
    const verified = await loadPublicLivePublishBaseline();
    if (!semanticValuesEqual(normalizePublishedBaseline(verified.snapshot), normalizePublishedBaseline(snapshot))) {
      throw new Error('Public read-back did not match the saved normalized snapshot.');
    }
    adminLiveRevision = Number(result.siteRevision) || adminLiveRevision + 1;
    updateAdminLiveSettings({
      lastPublishedSnapshot: structuredClone(snapshot),
      liveContentEnabled: true,
      liveContentRevision: Number(verified.liveRevision) || Number(result.liveRevision) || 0,
      livePublishedAt: String(verified.publishedAt || result.publishedAt || new Date().toISOString())
    });
    adminPublishedSettingsDocument = structuredClone(verified);
    adminPublishedBaseline = normalizePublishedBaseline(verified.snapshot);
    adminLastSuccessfulSnapshot = structuredClone(verified.snapshot);
    adminPublishedFileState = {
      reachable: true,
      publishedAt: String(verified.publishedAt || ''),
      commitHash: '',
      source: 'supabase-live'
    };
    currentPublishReview = null;
    refreshVisibleAdminAreaAfterPublish();
    report('LIVE — customer website content verified.', 'published');
    return true;
  } catch (error) {
    const activationPending = ['UNKNOWN_ACTION', 'LIVE_CONTENT_NOT_ACTIVATED', 'PGRST202'].includes(String(error?.code || ''))
      || /not activated|unknown publisher action|function .* does not exist/i.test(String(error?.message || ''));
    const detail = activationPending
      ? 'Fast live content awaits the separately approved Supabase activation.'
      : String(error?.message || 'The public value could not be persisted.');
    report(`SAVE FAILED — WEBSITE NOT CHANGED. ${detail}`, 'failed');
    return false;
  }
}
...
async function saveLiveChangeIds(changeIds, label = 'Saved Admin change', statusTarget = null, { onProgress = null } = {}) {
  ...
  let selected = architectureReviewItems().filter((item) => changeIds.includes(item.id));
  if (!selected.length) {
  ...
  return savePublicLiveSnapshot(buildSelectedArchitectureSnapshot(selected), label, statusTarget, { onProgress });
}
...
async function saveAllOpenAdminChanges({ quiet = false } = {}) {
  ...
  for (const form of document.querySelectorAll('.admin-product-card[data-slug]')) {
    ...
    if (!await saveProductForm(form)) {
      if (!quiet) setStatus(`Save All stopped at Product / Standee ${form.dataset.slug}.`);
      return false;
    }
  }
  for (const form of document.querySelectorAll('.admin-image-draft[data-image-box-dirty="true"]')) {
    if (!await saveImageBoxProductDraft(form)) {
      if (!quiet) setStatus(`Save All stopped at Image Box ${form.dataset.imagePath}.`);
      return false;
    }
  }
  return waitForAdminSaves();
}

async function saveAllLiveChanges(label = 'All intended Admin changes', statusTarget = null) {
  const report = (message, state = '') => {
    if (statusTarget) {
      statusTarget.textContent = message;
      statusTarget.dataset.state = state;
  ...
  report('Saving every dirty open Admin editor privately…', 'saving-live');
  if (!await saveAllOpenAdminChanges({ quiet: true })) {
    report('SAVE FAILED — WEBSITE NOT CHANGED. An open editor could not be saved.', 'failed');
    return false;
  }
  if (!await loadAdminLiveSettings()) {
    report('SAVE FAILED — WEBSITE NOT CHANGED. Saved Admin state could not be reloaded.', 'failed');
    return false;
  }
  const items = architectureReviewItems();
  if (!items.length) {
    report('LIVE — there are no saved changes waiting to go live.', 'published');
    return true;
  }
  const failed = await prepareArchitectureItemsForLive(items, report);
  if (failed.length) {
    report(`SAVE FAILED — WEBSITE NOT CHANGED. Could not prepare: ${failed.join('; ')}.`, 'failed');
    return false;
  }
  if (!await loadAdminLiveSettings()) {
    report('SAVE FAILED — WEBSITE NOT CHANGED. Prepared Admin state could not be reloaded.', 'failed');
    return false;
  }
  const ready = architectureReviewItems();
  return savePublicLiveSnapshot(
    buildSelectedArchitectureSnapshot(ready),
    `${label} (${ready.length})`,
    statusTarget
  );
}
...
async function publishScopedChangeIds(changeIds, label, statusTarget = null, { onProgress = null } = {}) {
  ...
  if (!persisted) {
    setLocalStatus(`Publish stopped: ${adminLastSaveError || 'private state could not be reloaded.'}`, 'failed');
    return false;
  }
  ...
  if (newAdminArchitectureEnabled() && unresolved) {
    setLocalStatus(`Publish stopped: ${unresolved} conflict${unresolved === 1 ? '' : 's'} require attention in Advanced.`, 'failed');
    return false;
  }
  ...
  const summary = changes.map((change) => `- ${change}`).join('\n');
  if (!window.confirm(`Publish “${label}”?\n\nChanges:\n${changes.join('\n')}`)) {
    setLocalStatus('Publish canceled. Your private draft is still saved.', 'draft');
    return false;
  }
  try {
    ...
    adminPublishedFileState = { reachable: true, publishedAt: result.publishedAt || new Date().toISOString(), commitHash: result.commitHash || '' };
    currentPublishReview = null;
    ...
    renderAdminDiagnostics();
    setLocalStatus(`Publish Failed — ${adminLatestPublishError}`, 'failed');
    return false;
  }
}
...
async function publishSavedProductBySlug(slug, label, statusTarget = null, form = null) {
  ...
  return saveLiveChangeIds([`product:${slug}`], label || base.title || slug, statusTarget);
}
...
async function publishNewProductFromForm(form) {
...
async function publishNewCategoryFromForm(form) {
  const formData = new FormData(form);
  const key = makeSlug(formData.get('key') || formData.get('title'));
  const title = String(formData.get('title') || key);
  const button = form.querySelector('[data-publish-new-category]');
  if (button) button.disabled = true;
  try {
    if (!await saveNewCategoryFromForm(form, 'draft')) return false;
    return saveLiveChangeIds([`category:${key}`], title, form.querySelector('[data-create-category-status]'));
  } finally {
    if (button) button.disabled = false;
  }
}
...
async function requestAdminContentSuggestion(form, action, button) {
  ...
  try {
    ...
    const formData = new FormData(form);
    const existingProduct = effectiveAdminProduct(String(formData.get('parentProductSlug') || ''));
    const existingCategory = readAdminCategories()[String(form.dataset.categoryEdit || '')];
      ...
      body: JSON.stringify({
        ...
        context: {
          ...
          existingCategory: existingCategory ? {
            ...
            funFact: existingCategory.funFact
          } : null
        }
      }),
...
async function saveNewProductFromForm(form, approvalStatus) {
  ...
  setCreationStatus(form, approvalStatus === 'approved' ? 'Unpublished Changes' : 'Draft saved privately.', 'saved');
  form.reset();
...
async function saveNewCategoryFromForm(form, approvalStatus) {
  ...
  if (approvalStatus === 'approved' && !String(formData.get('cardImage') || '').trim()) {
  ...
  if (equivalents.length) {
    ...
    const notice = document.getElementById('adminCategoryDuplicateNotice');
    if (notice) {
      notice.hidden = false;
      notice.innerHTML = `<strong>A similar Category already exists.</strong><p>${escapeAdminHtml(existing.title || existing.key)} — <code>${escapeAdminHtml(existing.key)}</code></p><div class="admin-panel-actions"><button type="button" data-open-existing-category="${escapeAdminHtml(existing.key)}">Open Existing</button><button type="button" data-cancel-new-category>Cancel New Category</button></div>`;
    }
    setCreationStatus(form, 'A similar Category already exists. Open the existing Category or cancel the new Category.', 'error');
    return false;
  }
  ...
  const result = await saveAdminCollectionOperations([
    ...
    { type: 'membership', collectionKey: 'deletedCategories', entryKey: key, present: false, baseValues: latest.edits?.deletedCategories || [] }
  ]);
  if (!result.ok) {
    setCreationStatus(form, adminLastSaveError || 'Save failed — your values remain on screen.', 'error');
    return false;
...
function setupAdminCreationWorkspace() {
  ...
  const categoryMount = document.getElementById('adminCategoryBuilderMount');
  if (categoryMount && categoryForm.parentElement !== categoryMount) categoryMount.appendChild(categoryForm);
  ...
  populateNewCategoryVisualPickers(categoryForm);
  [productForm, categoryForm].forEach((form) => {
    ...
    if (availability) availability.textContent = enabled
      ...
      : 'Preview available. Open Advanced to finish activating the private Admin architecture before saving.';
  });
  ...
  productForm.querySelector('[data-publish-new-product]')?.addEventListener('click', () => publishNewProductFromForm(productForm));
  categoryForm.querySelector('[data-preview-new-category]')?.addEventListener('click', () => renderNewCategoryPreview(categoryForm));
  categoryForm.querySelector('[data-publish-new-category]')?.addEventListener('click', () => publishNewCategoryFromForm(categoryForm));
  productForm.addEventListener('submit', (event) => {
    event.preventDefault();
    if (!newAdminArchitectureEnabled()) {
      setCreationStatus(productForm, 'Open Advanced to activate the private Admin architecture before saving.', 'error');
      return;
    }
  ...
  categoryForm.addEventListener('submit', (event) => {
    event.preventDefault();
    if (!newAdminArchitectureEnabled()) {
      setCreationStatus(categoryForm, 'Open Advanced to activate the private Admin architecture before saving.', 'error');
      return;
    }
...
function collectProductFormData(form, dirtyFields = form?._adminDirtyFields || new Set()) {
  ...
  const patch = {};
  const textFields = ['title', 'description', 'funFact', 'cutoutImage', 'backgroundImage', 'originalHeight', 'priceOverride', 'cutoutHeight', 'cutoutLeft', 'cutoutBottom', 'logoWidth', 'logoTop', 'stageBackgroundPosition'];
...
async function saveProductForm(form, message = 'DRAFT SAVED — PRIVATE. Use Save Live when customers should see the saved work.') {
...
async function handleImageUpload(fileInput, targetInput, form) {
  ...
  fileInput.value = '';
  setProductSaveState(form, 'Local files are not embedded. Add the file to images/, then choose it through Image Inbox.', 'draft');
  setStatus('Image not saved. Add the file to the repository images/ folder, open Image Inbox, and assign the repository path. Embedded Base64 images are blocked.');
}
...
function categoryAssignmentMarkup(value) {
  const categories = categoryDefinitionsForAdmin().filter((category) => !readDeletedCategories().includes(category.key));
        ...
        <div class="admin-category-options">
          ${categories.map((category) => `
            <label>
              ...
              ${category.title || category.label || category.key}
            </label>
          `).join('')}
        </div>
      </details>
      <div class="admin-category-order-row">
        <label>
          ...
          <select name="activeCategory">
            ${categories.map((category) => `<option value="${category.key}" ${selected.has(category.key) ? 'selected' : ''}>${category.title || category.label || category.key}</option>`).join('')}
          </select>
        </label>
...
function productCategoryNames(product) {
  const labels = new Map(categoryDefinitionsForAdmin().map((category) => [category.key, category.title || category.label || category.key]));
...
function bindParentProductPicker(scope) {
  ...
  const updatePreview = () => {
    ...
    const imageDraftForm = scope.matches?.('.admin-image-draft') ? scope : null;
    if (imageDraftForm && product && imageDraftForm.dataset.prefilledProduct !== product.slug) {
      ['title', 'description', 'funFact'].forEach((fieldName) => {
        const field = imageDraftForm.elements.namedItem(fieldName);
        if (field) field.value = String(product[fieldName] || '');
      });
      imageDraftForm.dataset.prefilledProduct = product.slug;
    }
  };
...
function architectureReviewItems() {
  ...
  Object.entries(readAdminProducts()).forEach(([slug, product]) => {
    ...
    if (semanticProductEqual(before || null, after)) return;
    items.push({
      id: `product:${slug}`,
  ...
  readDeletedCategories().forEach((key) => {
    if ((baseline.deletedCategories || []).includes(key)) return;
    const before = baseline.categories?.[key]
      || adminArchitectureState?.candidate?.categories?.[key]
      || { key, title: key };
    items.push({
      id: `category-delete:${key}`,
      type: 'category-delete', key,
      group: 'Category Deletions',
      title: before.title || key,
      approved: true,
      updatedAt: '',
      before,
      after: null,
      page: before.page || 'index.html'
    });
  });
  Object.entries(adminPageLiveEdits || {}).forEach(([pageKey, entries]) => {
    Object.entries(entries || {}).forEach(([elementKey, edit]) => {
      ...
      if (semanticValuesEqual(before, after)) return;
      items.push({
...
function buildSelectedArchitectureSnapshot(items) {
  ...
  (items || []).forEach((item) => {
    ...
    if (item.type === 'category') {
      ...
      baseline.deletedCategories = (baseline.deletedCategories || []).filter((key) => key !== item.key);
    ...
    if (item.type === 'category-delete') {
      delete baseline.categories[item.key];
      const cardSlug = categoryCardSlugForKey(item.key);
      delete baseline.categoryDisplayCards[cardSlug];
      delete baseline.categoryDisplayCards[`${item.key}-category-card`];
      delete baseline.categorySettings?.[item.key];
      baseline.deletedCategories = [...new Set([...(baseline.deletedCategories || []), item.key])].sort();
      Object.values(baseline.products || {}).forEach((product) => {
        product.categories = (product.categories || []).filter((key) => key !== item.key);
        if (product.categoryOrder) delete product.categoryOrder[item.key];
      });
      baseline.homepageCategoryOrder = (baseline.homepageCategoryOrder || []).map((row) => row.filter((value) => (
        value !== item.key && value !== cardSlug && value !== `${item.key}-category-card`
      )));
      return;
    }
    if (item.type === 'page') {
...
function automaticPublishImagePaths(items, snapshot) {
  ...
  return [...publishSnapshotImagePaths(snapshot)]
    .filter((path) => !publishedPaths.has(path))
    .filter((path) => imageInventorySource !== 'local' || localOnlyImagePaths.has(path) || !repositoryImagePaths.has(path))
    .sort();
}
...
function readyPublishSelectionMarkup(items) {
  ...
  return `
    <div class="admin-panel-actions">
      <button class="admin-button admin-button-primary" type="button" data-publish-all-saved>Publish Static Backup / New Assets</button>
    </div>
...
function redactAdminEmbeddedImages(item) {
  if (typeof item === 'string' && /^(?:data:image\/|blob:|admin-upload:)/i.test(item)) return '[Legacy embedded image omitted]';
  if (Array.isArray(item)) return item.map(redactAdminEmbeddedImages);
  if (!item || typeof item !== 'object') return item;
  return Object.fromEntries(Object.entries(item).map(([key, entry]) => [key, redactAdminEmbeddedImages(entry)]));
}

function architectureReviewSummary(value) {
  const text = JSON.stringify(redactAdminEmbeddedImages(value ?? null));
  return text.length > 180 ? `${text.slice(0, 177)}…` : text;
}

function architectureReviewImage(item) {
  const reference = item.type === 'category'
    ? item.after?.card?.image || item.before?.card?.image || ''
    : item.after?.cutoutImage || item.before?.cutoutImage || '';
  return adminStateUtils.validateAdminImageReference(reference, { allowBlank: true }).valid ? reference : '';
}
...
function architectureReviewMarkup(items) {
  ...
  return `<div class="admin-panel-actions"><button class="admin-button admin-button-primary" type="button" data-publish-all-saved>Publish Static Backup / New Assets</button></div>` + Object.entries(groups).map(([group, groupItems]) => `
...
function categoryLifecycleState(category) {
  const published = adminPublishedBaseline?.categories?.[category.key];
  const current = publishableCategory(category);
  if (!published) {
    if (category.approvalStatus === 'draft' || category.draftStatus === 'draft') return 'Draft';
...
function categoryAssignedProducts(categoryKey) {
  return effectiveAdminProducts()
    .filter((product) => (product.categories || []).includes(categoryKey))
    .sort((left, right) => String(left.title || left.slug).localeCompare(String(right.title || right.slug)));
}

function categoryAssignmentOptions(product) {
  const selected = new Set(product.categories || []);
  return categoryDefinitionsForAdmin()
    .filter((category) => category?.key && !readDeletedCategories().includes(category.key))
    .sort((left, right) => Number(left.order || 0) - Number(right.order || 0))
    .map((category) => `<label><input type="checkbox" name="categories" value="${escapeAdminHtml(category.key)}" ${selected.has(category.key) ? 'checked' : ''}> ${escapeAdminHtml(category.title || category.key)}</label>`)
    .join('');
}
...
function categoryChildGroups(categoryKey, categories = readAdminCategories()) {
  return adminStateUtils?.childCategories
    ? adminStateUtils.childCategories(categories, categoryKey)
...
function preferredCategoryImages(category, kind = 'category') {
  ...
  if (kind === 'background') {
    add(category.card?.backgroundImage);
    add(category.displaySettings?.backgroundImage);
    add(IMAGE_IMPORT_DEFAULT_BACKGROUND);
  } else add(category.card?.image);
  categoryAssignedProducts(category.key).forEach((product) => {
    if (kind === 'background') add(product.backgroundImage);
    else {
      add(product.cutoutImage);
      (product.imageChoices || []).forEach((choice) => add(choice.image || choice.src));
...
function adminImageReferencePresentation(value, { background = false } = {}) {
  const reference = String(value || '').trim();
  const validation = adminStateUtils.validateAdminImageReference(reference, { allowBlank: true });
  if (!reference) return {
    reference: '', valid: true,
    label: background ? 'Shared default background' : 'No image selected',
    preview: background ? IMAGE_IMPORT_DEFAULT_BACKGROUND : ''
  };
  if (!validation.valid) return {
    reference, valid: false,
    label: /^data:image\//i.test(reference) ? 'Legacy Base64 image' : 'Invalid embedded image',
    preview: background ? IMAGE_IMPORT_DEFAULT_BACKGROUND : ''
  };
  return { reference, valid: true, label: creationImageLabel(reference), preview: reference };
}
...
function populateNewCategoryVisualPickers(form) {
  const draft = { key: '__new-category__', card: {}, displaySettings: {} };
...
function categoryCardDraftStatusMarkup(category) {
  ...
  const lifecycleMessage = unpublished
    ? `DRAFT PREVIEW — NOT LIVE YET · SAVED PRIVATELY · LIVE VERSION STILL SAYS: “${publishedTitle}” · USE SAVE LIVE WHEN READY`
    : 'LIVE';
...
function categoryFromEditForm(form, approvalStatus = 'draft') {
  ...
  const current = readAdminCategories()[key] || {};
  const data = new FormData(form);
  const cardImageInput = form.elements.namedItem('cardImage');
  const backgroundImageInput = form.elements.namedItem('cardBackgroundImage');
...
async function publishCategoryByKey(categoryKey, form = null) {
  if (!categoryKey || ['publishing', 'saving-live'].includes(categoryPublishOperations.get(categoryKey)?.state)) return false;
...
async function saveCategoryProductAssignments(form, removeCurrent = false) {
  const slug = form.dataset.categoryProduct;
  const currentCategory = form.dataset.categoryKey;
  const product = effectiveAdminProduct(slug);
  if (!product) return false;
  const selected = removeCurrent
...
async function deleteAdminCategories(keys) {
  const categoryKeys = [...new Set(keys || [])].filter((key) => readAdminCategories()[key]);
  ...
  const details = categoryKeys.map((key) => {
    const products = categoryAssignedProducts(key);
  ...
  categoryKeys.forEach((key) => {
    operations.push({ type: 'record', collectionKey: 'categories', entryKey: key, baseRecord: readAdminCategories()[key], remove: true });
    operations.push({ type: 'membership', collectionKey: 'deletedCategories', entryKey: key, present: true, baseValues: readDeletedCategories() });
  });
  products.forEach((product) => {
    if (!(product.categories || []).some((key) => categoryKeys.includes(key))) return;
    const updated = adminStateUtils.withProductCategories(product, (product.categories || []).filter((key) => !categoryKeys.includes(key)));
    operations.push({ type: 'record', collectionKey: 'products', entryKey: product.slug, baseRecord: readAdminProducts()[product.slug], patch: { categories: updated.categories, categoryOrder: updated.categoryOrder, updatedAt: new Date().toISOString(), draftStatus: 'draft', approvalStatus: 'draft' } });
  });
  const result = await saveAdminCollectionOperations(operations);
  if (!result.ok) return false;
  renderAdminProducts();
...
async function recreateDeletedCategory(key) {
  const source = adminPublishedBaseline?.categories?.[key]
    || adminArchitectureState?.candidate?.categories?.[key]
    || { key, title: key, description: '', page: `category.html?category=${encodeURIComponent(key)}`, visible: true, homepageVisible: true, order: 999, card: {}, displaySettings: {} };
  const duplicates = adminStateUtils.findEquivalentCategories(readAdminCategories(), source, key);
  if (duplicates.length) {
    setStatus(`A similar Category already exists: ${duplicates.map((item) => `${item.title} (${item.key})`).join(', ')}. Open the existing Category or cancel recreation.`);
    return false;
  }
  const category = { ...structuredClone(source), key, approvalStatus: 'draft', draftStatus: 'draft', updatedAt: new Date().toISOString() };
  const result = await saveAdminCollectionOperations([
    { type: 'membership', collectionKey: 'deletedCategories', entryKey: key, present: false, baseValues: readDeletedCategories() },
    { type: 'record', collectionKey: 'categories', entryKey: key, baseRecord: undefined, patch: category }
  ]);
  if (!result.ok) return false;
  renderAdminProducts();
  setStatus(`Recreated ${category.title} as a private draft. Nothing was published.`);
  return true;
}

function previewCategoryEdit(form) {
...
function renderCategoryImagePickerGallery(picker, query = '', searchAll = false) {
  const category = readAdminCategories()[picker.dataset.categoryKey] || { key: picker.dataset.categoryKey, card: {}, displaySettings: {} };
...
function updateCategoryPickerValue(picker, path) {
  ...
  if (!input) return;
  input.value = path;
  input.dataset.preserveInvalidReference = 'false';
  const isBackground = picker.dataset.imageKind === 'background';
  const presentation = adminImageReferencePresentation(path, { background: isBackground });
  const currentImage = picker.querySelector('[data-category-current-image]');
  const currentPath = picker.querySelector('[data-category-current-path]');
  const currentLabel = picker.querySelector('[data-category-current-label]');
  if (currentImage) {
    if (presentation.preview && currentImage.tagName === 'IMG') currentImage.src = presentation.preview;
    else if (presentation.preview) currentImage.outerHTML = `<img src="${escapeAdminHtml(presentation.preview)}" alt="Current ${isBackground ? 'Category background' : 'Category image'}" data-category-current-image>`;
    else currentImage.outerHTML = '<span class="admin-category-no-image" data-category-current-image>No image</span>';
  }
  if (currentLabel) {
    currentLabel.textContent = presentation.label;
    currentLabel.dataset.invalidImage = String(!presentation.valid);
  }
...
function updateCategoryDraftPublishedState(form, category = categoryFromEditForm(form)) {
  ...
  if (state) {
    ...
    state.textContent = unpublished
      ? `${dirty ? 'UNSAVED CHANGES' : 'DRAFT SAVED — PRIVATE'} · LIVE VERSION STILL SAYS: “${publishedTitle}” · USE SAVE LIVE WHEN READY`
      : 'LIVE';
  }
  if (draftReference) {
...
function setupCategoryManagerEvents() {
  ...
  document.querySelector('[data-new-category]')?.addEventListener('click', () => {
    ...
    document.getElementById('createCategoryForm')?.scrollIntoView({ behavior: 'smooth', block: 'start' });
  });
  ...
  section.addEventListener('click', async (event) => {
    ...
    const changeImage = event.target.closest('[data-change-category-image]');
    if (changeImage) {
    ...
    if (publishDeletion) {
      const key = publishDeletion.dataset.publishCategoryDeletion;
      await saveLiveChangeIds([`category-delete:${key}`], `Delete ${key}`);
    }
    ...
    if (openExisting) {
      const search = document.getElementById('adminCategorySearch');
      if (search) search.value = openExisting.dataset.openExistingCategory;
      renderCategoryManager();
      document.querySelector(`[data-category-card="${CSS.escape(openExisting.dataset.openExistingCategory)}"]`)?.scrollIntoView({ behavior: 'smooth', block: 'start' });
    }
    if (event.target.closest('[data-cancel-new-category]')) {
...
function renderAdminProducts() {
  ...
  const productMarkup = (products, lifecycleLabel = '') => products.map((product) => {
    const storedValue = { ...product, ...(saved[product.slug] || {}) };
    const cutoutPresentation = adminImageReferencePresentation(storedValue.cutoutImage);
    const backgroundPresentation = adminImageReferencePresentation(storedValue.backgroundImage, { background: true });
    const invalidLegacyImages = [cutoutPresentation, backgroundPresentation].filter((item) => !item.valid);
    const value = {
      ...storedValue,
      cutoutImage: cutoutPresentation.valid ? storedValue.cutoutImage : '',
      ...
      <form class="admin-product-card" id="product-${product.slug}" data-slug="${product.slug}" data-product-search-value="${escapeAdminHtml(`${value.title || product.title} ${product.slug} ${productCategoryNames(value)}`.toLowerCase())}">
        <div class="admin-product-heading">
          ...
          <div class="admin-card-actions">
            ...
            ${product.categoryCard ? '' : '<button class="admin-button admin-button-primary" type="button" data-publish-product>Save Live</button>'}
        ...
        <div class="admin-product-layout">
          ...
          <div class="admin-control-groups">
            <fieldset>
              ...
              <label>
                Fun fact
                <textarea name="funFact" rows="2">${value.funFact || ''}</textarea>
              </label>
            </fieldset>
  ...
  const productSummaryMarkup = (products) => products.map((product) => {
    const value = { ...product, ...(saved[product.slug] || {}) };
    const lifecycle = productLifecycleState(product, saved, archived).label;
    const previewPage = readAdminCategories()[value.categories?.[0]]?.page || 'index.html#shop';
    const editorOpen = openedProductEditors.has(product.slug);
    const imagePresentation = adminImageReferencePresentation(value.cutoutImage);
    return `<article class="admin-product-summary-card" data-product-summary="${escapeAdminHtml(product.slug)}" data-product-search-value="${escapeAdminHtml(`${value.title || product.title} ${product.slug} ${productCategoryNames(value)}`.toLowerCase())}">
      <div class="admin-review-image">${imagePresentation.preview ? `<img src="${escapeAdminHtml(imagePresentation.preview)}" alt="" loading="lazy">` : `<span>${escapeAdminHtml(imagePresentation.label)}</span>`}</div>
      <div><h3>${escapeAdminHtml(value.title || product.title)}</h3><code>${escapeAdminHtml(product.slug)}</code><p>${escapeAdminHtml(productCategoryNames(value))}</p><strong>${escapeAdminHtml(lifecycle)}</strong></div>
      <div class="admin-card-actions"><button type="button" data-edit-product="${escapeAdminHtml(product.slug)}">Edit</button><a class="admin-button admin-button-secondary" href="${escapeAdminHtml(adminPreviewUrl(previewPage))}" target="_blank" rel="noopener">Preview</a></div>
      <div class="admin-product-editor-mount" data-product-editor-mount>${editorOpen ? productMarkup([product]) : ''}</div>
    </article>`;
  }).join('');

  const approvedProducts = availableProducts.filter((product) => !product.categoryCard);
  if (approvedContainer && ['loading', 'loaded'].includes(adminAreaLoadState.get('products')?.status)) approvedContainer.innerHTML = productSummaryMarkup(approvedProducts);
  if (publishedContainer && ['loading', 'loaded'].includes(adminAreaLoadState.get('advanced')?.status)) {
  ...
  if (['loading', 'loaded'].includes(adminAreaLoadState.get('products')?.status)) approvedContainer?.querySelectorAll('[data-edit-product]').forEach((button) => {
    button.addEventListener('click', () => {
      openedProductEditors.add(button.dataset.editProduct);
      renderAdminProducts();
      document.querySelector(`[data-product-summary="${CSS.escape(button.dataset.editProduct)}"] .admin-product-card`)?.scrollIntoView({ behavior: 'smooth', block: 'start' });
    });
  });
  ...
  productContainers.forEach((container) => container.querySelectorAll('.admin-product-card').forEach((form) => {
    ...
    form.querySelector('[data-preview-product]')?.addEventListener('click', () => {
      syncPreviewFromFields(form);
      form.querySelector('.admin-card-preview-wrap')?.scrollIntoView({ behavior: 'smooth', block: 'center' });
      setProductSaveState(form, 'Preview updated. Nothing was saved.', 'draft');
    });
  ...
  filterAdminProductLibrary();
}

function filterAdminProductLibrary() {
  const query = document.getElementById('adminProductSearch')?.value.trim().toLowerCase() || '';
  document.querySelectorAll('#approvedProducts .admin-product-summary-card').forEach((card) => {
    card.hidden = Boolean(query) && !String(card.dataset.productSearchValue || '').includes(query);
  });
}
...
const IMAGE_IMPORT_ROLES = [
  ['main', 'Main Image'],
  ['image-choice', 'Additional Image / Version'],
...
function imageDraftCategoryMarkup(selectedCategories = []) {
  ...
  return categoryDefinitionsForAdmin().filter((category) => !readDeletedCategories().includes(category.key)).map((category) => `
    <label>
      <input name="categories" type="checkbox" value="${category.key}" ${selected.has(category.key) ? 'checked' : ''}>
      ${category.title || category.label || category.key}
    </label>
...
async function configureImageDraft(form, approvalStatus = 'draft') {
  ...
  try {
    if (draft.destination === 'existing-product') {
      ...
      updated.draftStatus = approvalStatus === 'draft' ? 'draft' : 'ready';
...
async function handleImageInboxAction(form, action) {
  ...
  if (!await saveImageBoxProductDraft(form)) {
    setImageDraftActionStatus(form, 'SAVE FAILED — WEBSITE NOT CHANGED. Latest changes could not be saved privately.', 'error');
    return false;
  }
  const draft = collectImageDraftForm(form);
  ...
  const product = effectiveAdminProduct(slug);
  if (!product) {
    setImageDraftActionStatus(form, 'SAVE FAILED — WEBSITE NOT CHANGED. The saved normalized Product / Standee was not found.', 'error');
    return false;
  }
  const status = form.querySelector('[data-image-draft-status]');
  setImageDraftActionsBusy(form, true);
  setImageDraftActionStatus(form, 'SAVING LIVE…', 'publishing');
  try {
    ...
    if (published && status?.dataset.state === 'published') {
      markImageBoxSaved(form);
      setImageDraftActionStatus(form, 'LIVE', 'published');
    }
    return published;
  } finally {
    setImageDraftActionsBusy(form, false);
...
function updateImageDraftDestination(form) {
  ...
  const role = String(values.get('imageRole') || 'main');
...
function imageImportPreviewChoices(form) {
  ...
  const importRole = String(new FormData(form).get('imageRole') || 'main');
  const choices = [{
...
function renderImageDrafts() {
  ...
  effectiveAdminProducts().forEach((product) => {
    ...
    if (product.backgroundImage) hiddenPaths.add(product.backgroundImage);
    normalizeImageChoices(product.imageChoices).forEach((choice) => {
      hiddenPaths.add(choice.image);
      if (choice.stage) hiddenPaths.add(choice.stage);
    });
  });
  Object.values(readAdminCategories()).forEach((category) => {
  ...
  container.innerHTML = drafts.map((inventoryDraft) => {
    ...
    if (!openedImageInboxItems.has(draft.path)) return `
      <article class="admin-image-inbox-summary" data-image-inbox-summary="${escapeAdminHtml(draft.path)}">
        ...
        <div><strong>${escapeAdminHtml(draft.title || creationImageLabel(draft.path))}</strong><span>${escapeAdminHtml(creationImageLabel(draft.path))}</span></div>
      ...
      <form class="admin-product-card admin-image-draft" data-image-path="${escapeAdminHtml(draft.path)}" data-product-slug="${escapeAdminHtml(draft.resultSlug || '')}" data-draft-status="${escapeAdminHtml(draft.status)}" data-saved-for-later="${draft.savedForLater === true}" data-image-box-dirty="${normalizedProductSaved ? 'false' : 'true'}">
        ...
        <div class="admin-product-layout">
          ...
          <div class="admin-control-groups">
            ...
            <fieldset><legend>What do you want to do?</legend>
              ${IMAGE_IMPORT_DESTINATIONS.map(([key, label]) => `<label class="admin-radio-option"><input name="imageDestination" type="radio" value="${key}" ${draft.destination === key ? 'checked' : ''}> ${label}</label>`).join('')}
            </fieldset>
            <fieldset data-import-destinations="existing-product"><legend>What is this image?</legend>
              ${IMAGE_IMPORT_ROLES.map(([key, label]) => `<label class="admin-radio-option"><input name="imageRole" type="radio" value="${key}" ${draft.imageRole === key ? 'checked' : ''}> ${label}</label>`).join('')}
            </fieldset>
            <div data-import-destinations="create-product existing-product">
            ...
            <div data-import-destinations="existing-product">
              ...
              <label data-import-roles="image-choice">Label<input name="imageChoiceLabel" type="text" value="${escapeAdminHtml(draft.imageChoiceLabel || '')}" placeholder="White Outfit, Black Outfit, Red Jersey, Alternate Pose"></label>
  ...
  container.querySelectorAll('[data-configure-image-inbox]').forEach((button) => {
    button.addEventListener('click', () => {
      openedImageInboxItems.add(button.dataset.configureImageInbox);
      renderImageDrafts();
      document.querySelector(`[data-image-path="${CSS.escape(button.dataset.configureImageInbox)}"]`)?.scrollIntoView({ behavior: 'smooth', block: 'start' });
    });
  });
...
function imageImportPublished(draft) {
  const snapshot = adminPublishedBaseline || adminLastSuccessfulSnapshot;
...
function imageImportStatus(draft) {
  if (draft.lastError || draft.status === 'error') return 'Publish Failed';
  if (imageImportPublished(draft)) return 'Published';
  if (draft.savedForLater) return 'Draft';
  if (['ready', 'completed'].includes(draft.status)) return 'Unpublished Changes';
  return 'Draft';
}
...
async function loadImageDraftInventory({ renderInbox = true } = {}) {
  try {
    ...
    try {
      ...
      repositoryImagePaths = new Set(imageDraftInventory.map((entry) => entry?.path).filter(Boolean));
      localOnlyImagePaths = new Set();
      imageInventorySource = 'fallback';
      setStatus(`Automatic image inventory is unavailable; showing the legacy read-only list. ${error?.message || error}`);
    } catch (_fallbackError) {
      imageDraftInventory = [];
    }
  }
  if (renderInbox) renderImageDrafts();
  imageInventoryLoaded = true;
}
...
function renderAdminDashboard() {
  ...
  const privateProducts = Object.values(readAdminProducts());
  const products = effectiveAdminProducts();
  const history = getAdminLiveValue('publishHistory', []);
  const comparisonContext = {
    globalDisplaySettings: adminPublishedBaseline?.globalDisplaySettings || {},
    categorySettings: adminPublishedBaseline?.categorySettings || {}
  };
  const productChangeCount = Object.entries(readAdminProducts()).filter(([slug, product]) => !adminStateUtils.semanticProductEqual(
    adminPublishedBaseline?.products?.[slug] || null,
    publishableProduct(product, readArchivedProducts().includes(slug)),
    comparisonContext
  )).length;
  const available = (value) => value === undefined || value === null || value === '' ? 'Not available' : value;
  const cards = [
    { title: 'Products', value: products.length, note: 'Published products and private product records.', href: '#products' },
    { title: 'Drafts', value: privateProducts.filter((item) => item.approvalStatus === 'draft' || item.draftStatus === 'draft').length, note: 'Saved privately; customers cannot see these changes.', href: '#products' },
    { title: 'Unpublished Changes', value: productChangeCount, note: 'Meaningful product changes that are not live yet.', href: '#products' },
    { title: 'Orders / Offers', value: adminCommerceLoaded ? 'Loaded' : 'Open', note: 'Customer orders and offers requiring normal attention.', href: '#orders' },
    { title: 'Recent Publications', value: history.length, note: adminPublishedFileState.publishedAt ? `Last: ${new Date(adminPublishedFileState.publishedAt).toLocaleDateString()}` : 'No publication recorded yet.', href: '#advanced' },
...
function renderAdminRecoveryTools() {
  ...
  container.innerHTML = `
    ...
    <div class="admin-panel-actions">
      ...
      ${!newAdminArchitectureEnabled() && prepared?.version === 1 ? '<button type="button" data-activate-admin-locally>Activate Private Admin Architecture</button>' : ''}
    ...
    <details><summary>Conflicting page overrides (${conflicts.length})</summary><pre>${escapeAdminHtml(JSON.stringify(redactAdminEmbeddedImages(conflicts), null, 2))}</pre></details>
    <details><summary>Technical published/private comparison</summary>${architectureReviewMarkup(architectureReviewItems())}</details>
...
function setAdminAreaLoading(area, loading, message = 'Loading…') {
  document.querySelectorAll(`[data-admin-area="${CSS.escape(area)}"]`).forEach((section, index) => {
    section.toggleAttribute('aria-busy', loading);
    section.querySelector(':scope > [data-admin-route-loading]')?.remove();
    if (!loading || index > 0) return;
    const notice = document.createElement('p');
    notice.className = 'admin-route-loading';
    notice.dataset.adminRouteLoading = '';
    notice.setAttribute('role', 'status');
    notice.textContent = message;
    const header = section.querySelector(':scope > .admin-panel-header');
    if (header) header.insertAdjacentElement('afterend', notice);
    else section.prepend(notice);
  });
}

async function ensureAdminAreaLoaded(area, { force = false } = {}) {
  const existing = adminAreaLoadState.get(area);
  if (!force && existing?.status === 'loaded') return existing.value;
  if (!force && existing?.status === 'loading') return existing.promise;
  const promise = Promise.resolve().then(async () => {
    setAdminAreaLoading(area, true, area === 'image-inbox' ? 'Loading repository image inventory…' : `Loading ${area.replace('-', ' ')}…`);
    if (area === 'dashboard') renderAdminDashboard();
    if (area === 'products') {
      await ensureAdminWorkingCollections(['categories', 'deletedCategories']);
      adminArchitectureState = await buildAdminArchitectureState(adminLiveSettings);
      const requestedProduct = window.location.hash.startsWith('#product-') ? window.location.hash.slice('#product-'.length) : '';
      if (requestedProduct) openedProductEditors.add(requestedProduct);
      setupAdminCreationWorkspace();
      renderAdminProducts();
      if (requestedProduct) document.getElementById(`product-${requestedProduct}`)?.scrollIntoView({ block: 'start' });
    }
    if (area === 'image-inbox') {
      await ensureAdminWorkingCollections(['categories', 'configuredImagePaths', 'deletedCategories', 'dismissedImageDrafts', 'extraImages', 'ignoredImagePaths', 'imageDrafts']);
      if (!imageInventoryLoaded || force) await loadImageDraftInventory();
    ...
    if (area === 'orders' && !adminCommerceLoaded) await refreshCommerceAdmin();
    if (area === 'settings') {
      await ensureAdminWorkingCollections(['coupons', 'extraImages', 'priceSettings']);
      setupPriceRules();
      renderExtraImages();
      setupCoupons();
      if (!adminTestModeLoaded) await loadAdminTestMode();
      if (!adminDiscountCodesLoaded) await loadDiscountCodes();
    }
    if (area === 'advanced') {
      await loadAdminRecoveryState();
      adminArchitectureState = await buildAdminArchitectureState(adminLiveSettings);
      renderAdminDiagnostics();
  ...
  adminAreaLoadState.set(area, { status: 'loading', promise });
  try {
    const value = await promise;
    adminAreaLoadState.set(area, { status: 'loaded', value });
    return value;
  } catch (error) {
    adminAreaLoadState.delete(area);
    setStatus(`Could not load ${area.replace('-', ' ')}. ${error?.message || error}`);
    throw error;
  } finally {
    setAdminAreaLoading(area, false);
  }
}
...
function showAdminAreaFromHash() {
  ...
  const aliases = {
    create: 'products', 'create-content': 'products', 'approved-products': 'products',
    'image-imports': 'image-inbox', 'new-image-drafts': 'image-inbox',
    'category-display-cards': 'categories', 'admin-settings': 'settings',
    publish: 'advanced', 'publish-changes': 'advanced', recovery: 'advanced', 'recovery-advanced': 'advanced'
  };
  ...
  if (!adminAccessReady) setAdminAreaLoading(area, true, 'Loading Admin data…');
  ...
  void ensureAdminAreaLoaded(area).catch(() => {});
}
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  const loadedAdminState = hasAdminAccess ? await loadAdminLiveSettings(ADMIN_DASHBOARD_COLLECTIONS, { replace: true }).catch(() => null) : null;
  ...
  document.getElementById('refreshImageDrafts')?.addEventListener('click', () => ensureAdminAreaLoaded('image-inbox', { force: true }));
  document.getElementById('saveSelectedImageDrafts')?.addEventListener('click', saveSelectedImageImportsForLater);
  document.getElementById('publishSelectedImageDrafts')?.addEventListener('click', openSelectedImageDraft);
  document.getElementById('adminProductSearch')?.addEventListener('input', filterAdminProductLibrary);
  document.querySelectorAll('[data-new-product]').forEach((link) => link.addEventListener('click', () => {
    window.setTimeout(() => document.getElementById('createProductForm')?.scrollIntoView({ behavior: 'smooth', block: 'start' }), 0);
  }));
  ...
  const saveLiveCategoryKey = new URLSearchParams(window.location.search).get('saveLiveCategory');
  if (saveLiveCategoryKey) {
    history.replaceState(null, '', 'admin.html#categories');
    window.setTimeout(() => publishCategoryByKey(saveLiveCategoryKey), 0);
  }
